...
/// PoToken (see potoken.js): a cryptographic attestation Google now
/// requires on /player for most content. Generated by the webview's own
/// JS (a real Chromium/WebView2/WKWebView engine can run Google's actual
/// BotGuard attestation script, which is the whole point - a headless
/// Rust process can't), then handed to Rust via `set_po_token`. A plain
/// module-level cell instead of managed Tauri state: this is Android-only
/// and effectively a process-wide singleton, so threading a `State`
/// through every free function in this file would just be noise.
#[derive(Clone)]
struct PoTokenData {
    po_token: String,
    visitor_data: String,
...
fn po_token_cell() -> &'static std::sync::Mutex<Option<PoTokenData>> {
    static CELL: std::sync::OnceLock<std::sync::Mutex<Option<PoTokenData>>> = std::sync::OnceLock::new();
    CELL.get_or_init(|| std::sync::Mutex::new(None))
}

fn current_po_token() -> Option<PoTokenData> {
    po_token_cell().lock().unwrap().clone()
}
...
fn player_client_cascade(po: Option<&PoTokenData>) -> Vec<Value> {
    ...
    // The PoToken is cryptographically bound to the visitorData it was
    // minted against - every client in the cascade needs that SAME
    // visitorData attached, or the token won't validate for any of them.
    if let Some(po) = po {
        for ctx in &mut ctxs {
            ctx["client"]["visitorData"] = json!(po.visitor_data);
        }
    }
    ctxs
}
...
/// media request, not just on /player - it has to be appended to the
/// stream URL itself as a query param, or the URL (even though it looked
/// "valid" in the API response) 403s the moment something tries to fetch it.
fn append_pot(url: &str, po: Option<&PoTokenData>) -> String {
    match po {
        Some(po) => {
            let sep = if url.contains('?') { '&' } else { '?' };
            format!("{url}{sep}pot={}", percent_encoding::utf8_percent_encode(&po.po_token, percent_encoding::NON_ALPHANUMERIC))
        }
        None => url.to_string(),
    }
}
